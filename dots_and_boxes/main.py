...
def execute_coup(coup, etat):
    ...
    sortie : le nouvel état de l'environnement, le nombre de case gagné en jouant le coup
    """
    dimension = dim(etat)
    (o,x,y) = coup
    ...
    if new_etat[o][x][y] != 0 :
        ...
        if o == 0 :
            ...
            if y > 0 : #il y a dimension[1] lignes horizontales, donc la dernière est à l'ordonnée y = dimension[1]
                if (new_etat[0][x][y-1] != 0 and new_etat[1][x][y-1] != 0 and new_etat[1][x+1][y-1] != 0 and  new_etat[2][x][y-1] == 0):
                    new_etat[2][x][y-1] = 1
                    nb_case_prise += 1
            #on teste le carré du dessous
            if y < dimension[1]-1 : 
                if (new_etat[0][x][y+1] != 0 and new_etat[1][x][y] != 0 and new_etat[1][x+1][y] != 0 and  new_etat[2][x][y] == 0):
                    new_etat[2][x][y] = 1
                    nb_case_prise += 1
        if o == 1 :
            #on teste le carré de gauche
            if x > 0 :
                if (new_etat[0][x-1][y] != 0 and new_etat[0][x-1][y+1] != 0 and new_etat[1][x-1][y] != 0 and  new_etat[2][x-1][y] == 0):
                    new_etat[2][x-1][y] = 1
                    nb_case_prise += 1
            #on teste le carré de droite
            if x < dimension[0]-1 : #il y a dimension[0] lignes verticales, donc la dernière est à l'abscisse x = dimension[0]
                if (new_etat[0][x][y] != 0 and new_etat[0][x][y+1] != 0 and new_etat[1][x+1][y] != 0 and  new_etat[2][x][y] ==0 ):
                    new_etat[2][x][y] = 1
                    nb_case_prise += 1
...
def execute_action(action,etat):
    ...
    sortie : le nouvel état de l'environnement, le nombre de case gagné en jouant l'action
    """
    plateau = etat
    for coup in action :
...
def clic_arete(plateau, coord, bouton):
    ...
    for i in range(dim_x):
        if coord[0] >= debutx + 0.25*interval + i*interval and coord[0] <= debutx + 0.25*interval + (i+0.5)*interval :
            testh = testh + 1
            arete_abscisse = i
    for j in range(dim_y):
        if coord[1] >= debuty - 0.25*interval + j*interval and coord[1] <= debuty - 0.25*interval + (j+0.5)*interval :
            testh = testh + 1
            arete_ordonee = j
    ...
    if testv == 2:
        if arete_ordonee_v <= dim_y-2 :
            if plateau[1][arete_abscisse_v][arete_ordonee_v] == 0  :
                draw_arete(False,plateau,1,arete_abscisse_v,arete_ordonee_v,couleur)
...
while run:
    ...
    while pvp :
        for pyEvent in event.get():
            ...
            if pyEvent.type == KEYDOWN and pyEvent.key == K_m :
                ...
                pvp = False
            if pyEvent.type == MOUSEMOTION:
                bouton_dimension_menu(screen,pyEvent.pos,choix_x,choix_y,color_fond)
            if pyEvent.type == MOUSEBUTTONDOWN:
                (choix_x,choix_y) = clic_bouton_dimension(pyEvent.pos,choix_x,choix_y)
    ...
    while rvr :
        for pyEvent in event.get():
            ...
            if pyEvent.type == KEYDOWN and pyEvent.key == K_m :
                ...
                rvr = False
            if pyEvent.type == MOUSEMOTION:
                bouton_robots_battle(screen,pyEvent.pos,num_robot1,num_robot2,color_fond)
            if pyEvent.type == MOUSEBUTTONDOWN:
                (num_robot1,num_robot2) = clic_robots_battle(screen,pyEvent.pos,num_robot1,num_robot2)
    ...
    while menu_color :
        for pyEvent in event.get():
            ...
            if pyEvent.type == MOUSEBUTTONDOWN:
                (choix_color_fond,choix_color_1,choix_color_2) = clic_menu_couleur(pyEvent.pos,choix_color_1,choix_color_2)
                if choix_color_1 != -1 and choix_color_2 != -1 or choix_color_fond != -1  :
                    menu_color = False
                    (color_fond,color_1,color_2) = attribue_color(color_fond,color_1,color_2,choix_color_fond,choix_color_1,choix_color_2)
                    choix_color_fond,choix_color_1,choix_color_2 = -1,-1,-1 
                    initialise_menu(screen,color_fond)
        display.flip()
    #boucle de partie :
        ...
        if mode_pvp :
            for pyEvent in event.get():
                ...
                if pyEvent.type == MOUSEBUTTONDOWN:
                    ...
                    # if fin_partie(plateau) :
                    #     if points_j1 > points_j2 :
                    #         message_de_fin = "Joueur 1 a gagné !"
                    #     elif points_j1 < points_j2 :
                    #         message_de_fin = "Joueur 2 a gagné !"
                    #     else :
                    #         "C'est une égalité !"
                    #     dim_x_rendu = police.render(message_de_fin, True, (255, 255, 255))
                    #     position_dim_x = dim_x_rendu.get_rect(center=(150, 150))
                    #     screen.blit(dim_x_rendu, position_dim_x) #texte pour les colonnes

                if  pyEvent.type == KEYDOWN and pyEvent.key == K_m :
                    initialise_menu(screen,color_fond)
                    joue = False
                    mode_pvp = False
                # if pyEvent.type == KEYDOWN and pyEvent.key == K_BACKSPACE :
                #     (plateau,tour_num) = retour(avancement,tour_num,color_fond,color_1,color_2)
