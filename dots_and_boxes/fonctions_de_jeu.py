...
def carre(plateau,c):
    ...
    effet : colorie en 'c' les carrés pouvant être coloriés (ceux non coloriés et entourés de 4 arêtes) 
    ...
    for i in range(dimension[0]-1):
        for j in range(dimension[1]-1):
            if(plateau[0][i][j] != 0 and plateau[0][i][j+1] !=0 and plateau[1][i+1][j] != 0 and plateau[1][i][j] != 0 and plateau[2][i][j] == 0) :
                if time_activation :
                    sleep((delay/10))
...
def test_danger(o,x,y,chaine):
    """
    entrée : le plateau sous forme [H,V,C], 3 entiers o,x,y formant une coordonné d'arête
    ...
    if o == 0 :
        ...
        for i in range(dimension[0]-1):
            for j in range(dimension[1]-1):
                if ((chaine[0][i][j] == 0 and chaine[0][i][j+1]==1 and chaine[1][i+1][j] == 1 and chaine[1][i][j] == 1 and chaine[2][i][j] == 0)
                    or (chaine[0][i][j] == 1 and chaine[0][i][j+1]==0 and chaine[1][i+1][j] == 1 and chaine[1][i][j] == 1 and chaine[2][i][j] == 0)
                    or (chaine[0][i][j] == 1 and chaine[0][i][j+1]==1 and chaine[1][i+1][j] == 0 and chaine[1][i][j] == 1 and chaine[2][i][j] == 0)
                    or (chaine[0][i][j] == 1 and chaine[0][i][j+1]==1 and chaine[1][i+1][j] == 1 and chaine[1][i][j] == 0 and chaine[2][i][j] == 0) ) :
                    trouve = trouve + 1
        chaine[0][x][y] = 0
...
def prochaine_arete(o,x,y,Chaine):
    """
    entrée :  3 entiers o,x,y formant une coordonné d'arête, le plateau sous forme [H,V,C]
    effet : trouve toutes le arêtes permettant de gagner un point après avoir joué l'arête (o,x,y)
    sortie : la liste des arêtes suivantes permettant de gagner un point sous forme [(o,x,y), ... ] avec potentiellement o = 2 si l'arête placée offre un carré directement.
    """
    Chaine[o][x][y] = 1
    dimension = dim(Chaine)
    list_res = []
    ...
    if o == 0 : #l'arête est horizontale
        if(y != 0): # elle n'est pas sur la ligne du haut : on teste le carré du dessus 
            # print("les arêtes du carré du dessus: ",Chaine[0][x][y-1],Chaine[1][x][y-1],Chaine[1][x+1][y-1])
            if ((Chaine[0][x][y-1] == 0) and (Chaine[1][x][y-1]==1) and (Chaine[1][x+1][y-1]==1) and (Chaine[2][x][y-1] == 0)): #ici l'arête horizontale du dessus
                list_res.append((0,x,y-1)) 
                # print("on a trouvé au dessus en ",0,",",x,",",y-1)
            if ((Chaine[0][x][y-1] == 1) and (Chaine[1][x][y-1]==0) and (Chaine[1][x+1][y-1]==1) and (Chaine[2][x][y-1] == 0)): # ici l'arête verticale de gauche
                list_res.append((1,x,y-1))
                # print("on a trouvé au dessus en ",1,",",x,",",y-1)
            if ((Chaine[0][x][y-1] == 1) and (Chaine[1][x][y-1]==1) and (Chaine[1][x+1][y-1]==0) and (Chaine[2][x][y-1] == 0)): # ici l'arête verticale de droite
                list_res.append((1,x+1,y-1))
                # print("on a trouvé au dessus en ",1,",",x+1,",",y-1)
            if ((Chaine[0][x][y-1] == 1) and (Chaine[1][x][y-1]==1) and (Chaine[1][x+1][y-1]==1) and (Chaine[2][x][y-1] == 0)): 
                list_res.append((2,x,y-1))
                # print("c'est la fin de la chaine de ce coté-ci ! (en haut)")      
                      

        if(y < dimension[1]-1): # elle n'est pas sur la ligne du bas : on teste le carré du dessous 
            # print("les arêtes du carré du dessous :",Chaine[0][x][y+1],Chaine[1][x][y],Chaine[1][x+1][y])
            if ((Chaine[0][x][y+1] == 0) and (Chaine[1][x][y]==1) and (Chaine[1][x+1][y]==1) and (Chaine[2][x][y] == 0)): #ici l'arête horizontale du dessus
                list_res.append((0,x,y+1))
                # print("on a trouvé en dessous en ",0,",",x,",",y+1)
            if ((Chaine[0][x][y+1] == 1) and (Chaine[1][x][y]==0) and (Chaine[1][x+1][y]==1) and (Chaine[2][x][y] == 0)): # ici l'arête verticale de gauche
                list_res.append((1,x,y))
                # print("on a trouvé en dessous en ",1,",",x,",",y)
            if ((Chaine[0][x][y+1] == 1) and (Chaine[1][x][y]==1) and (Chaine[1][x+1][y]==0) and (Chaine[2][x][y] == 0)): # ici l'arête verticale de droite
                list_res.append((1,x+1,y))
                # print("on a trouvé en dessous en ",1,",",x+1,",",y)
            if ((Chaine[0][x][y+1] == 1) and (Chaine[1][x][y]==1) and (Chaine[1][x+1][y]==1) and (Chaine[2][x][y] == 0)): 
                list_res.append((2,x,y))
                # print("c'est la fin de la chaine de ce coté-ci ! (en bas)")  


    else : #l'arete est verticale
        if (x != 0): #elle n'est pas tout à gauche : on teste le carré de gauche 
            # print("les arêtes du carré de gauche :",Chaine[0][x-1][y],Chaine[0][x-1][y+1],Chaine[1][x-1][y])
            if ((Chaine[0][x-1][y] == 0) and (Chaine[0][x-1][y+1]==1) and (Chaine[1][x-1][y]==1) and (Chaine[2][x-1][y] == 0)): # ici l'arête horizontale du dessus
                list_res.append((0,x-1,y))
                # print("on a trouvé à gauche en ",0,",",x-1,",",y)
            if ((Chaine[0][x-1][y] == 1) and (Chaine[0][x-1][y+1]==0) and (Chaine[1][x-1][y]==1) and (Chaine[2][x-1][y] == 0)): # ici l'arête horizontale du dessous
                list_res.append((0,x-1,y+1))
                # print("on a trouvé à gauche en ",0,",",x-1,",",y+1)
            if ((Chaine[0][x-1][y] == 1) and (Chaine[0][x-1][y+1]==1) and (Chaine[1][x-1][y]==0) and (Chaine[2][x-1][y] == 0)): # ici l'arête verticale de gauche
                list_res.append((1,x-1,y))
                # print("on a trouvé à gauche en ",1,",",x-1,",",y)
            if ((Chaine[0][x-1][y] == 1) and (Chaine[0][x-1][y+1]==1) and (Chaine[1][x-1][y]==1) and (Chaine[2][x-1][y] == 0)):
                list_res.append((2,x-1,y))
                # print("c'est la fin de la chaine de ce coté-ci ! (à gauche)")  

        if (x < dimension[0]-1): #elle n'est pas tout à droite : on teste le carré de droite 
            # print("les arêtes du carré de droite",Chaine[0][x][y],Chaine[0][x][y+1],Chaine[1][x+1][y])
            if ((Chaine[0][x][y] == 0) and (Chaine[0][x][y+1]==1) and (Chaine[1][x+1][y]==1) and (Chaine[2][x][y] == 0)): # ici l'arête horizontale du dessus
                list_res.append((0,x,y))
                # print("on a trouvé à droite en ",0,",",x,",",y)
            if ((Chaine[0][x][y] == 1) and (Chaine[0][x][y+1]==0) and (Chaine[1][x+1][y]==1) and (Chaine[2][x][y] == 0)): # ici l'arête horizontale du dessous
                list_res.append((0,x,y+1))
                # print("on a trouvé à droite en ",0,",",x,",",y+1)
            if ((Chaine[0][x][y] == 1) and (Chaine[0][x][y+1]==1) and (Chaine[1][x+1][y]==0) and (Chaine[2][x][y] == 0)): # ici l'arête verticale de droite
                list_res.append((1,x+1,y))
                # print("on a trouvé à droite en ",1,",",x+1,",",y)
            if ((Chaine[0][x][y] == 1) and (Chaine[0][x][y+1]==1) and (Chaine[1][x+1][y]==1) and (Chaine[2][x][y] == 0)):
                list_res.append((2,x,y))
...
def longueur_chaine(coordo,Chaine):
    """
    entrée : une liste d'arêtes sous forme [(o,x,y),...] et un plateau sous forme [H,V,C]
    ...
    -> une chaine 'fermée' est une chaine dont on peut gagner tous les carrés en un tour
    """
    res=0
    for i in range(len(coordo)): # pour toutes les arêtes à tester (on parcours un tableau de 3-uplets)
        (o,x,y) = coordo[i] # on extrait le ie 3-uplet
        if Chaine[o][x][y] == 0 : # si l'arête n'est pas prise ou déjà testée
            if o != 2 : 
                coordo_next = prochaine_arete(o,x,y,Chaine) # on récupère les arêtes intéressantes suivantes s'il y en a, et oxy est considérée comme prise
                # print(coordo_next)
...
def random_arete_dispo(plateau):
    ...
    sortie : renvoie (o,x,y) une arête libre au pif et renvoie (-1,0,0) s'il n'y en a plus
    """
    [a,b] = dim(plateau)
    for _ in range (100):
        o = randint(0,1)
...
def bool_phase_finale(plateau): 
    """
    entrée : un plateau sous forme [H,V,C]
    effet : renvoie un booléen signifiant si le plateau vérifie la définition de phase finale
    sortie : un booléen
    ...
    # print("avant la checking des arêtes sûres :")
    # pour_y_voir_plus_clair(chaine)
...
def fin_phase_finale(plateau): 
    ...
    while joue != 0 :
        ...
        for i in range(dimension[0]-1):
            for j in range(dimension[1]-1):
                if(plateau_test[0][i][j] == 0 and plateau_test[0][i][j+1]==1 and plateau_test[1][i+1][j] == 1 and plateau_test[1][i][j] == 1 and plateau_test[2][i][j] == 0) :
                    joue = joue + 1
        ...
        for i in range(dimension[0]-1):
            for j in range(dimension[1]-1):
                if(plateau_test[0][i][j] == 1 and plateau_test[0][i][j+1]==0 and plateau_test[1][i+1][j] == 1 and plateau_test[1][i][j] == 1 and plateau_test[2][i][j] == 0) :
                    joue = joue + 1
        ...
        for i in range(dimension[0]-1):
            for j in range(dimension[1]-1):
                if(plateau_test[0][i][j] == 1 and plateau_test[0][i][j+1]==1 and plateau_test[1][i+1][j] == 1 and plateau_test[1][i][j] == 0 and plateau_test[2][i][j] == 0) :
                    joue = joue + 1
        ...
        for i in range(dimension[0]-1):
            for j in range(dimension[1]-1):
                if(plateau_test[0][i][j] == 1 and plateau_test[0][i][j+1]==1 and plateau_test[1][i+1][j] == 0 and plateau_test[1][i][j] == 1 and plateau_test[2][i][j] == 0) :
                    joue = joue + 1
    ...
    for i in range(dimension[0]-1):
        for j in range(dimension[1]-1):
            if(plateau_test[0][i][j] == 1 and plateau_test[0][i][j+1]==1 and plateau_test[1][i+1][j] == 1 and plateau_test[1][i][j] == 1 and plateau_test[2][i][j] == 0) :
                plateau_test[2][i][j] = 1
...
def nb_arete_chaine(coordo,Chaine):
    ...
    effet : compte le nombre d'arêtes dans la chaine 'fermée' par l'arête de coordonée coordo = (o,x,y)
    sortie : le nombre d'arêtes dans la chaine 'fermée' par l'arête de coordonée coordo
    -> une chaine 'fermée' est une chaine dont on peut gagner tous les carrés en un tour
    """
    res = 0
    for i in range(len(coordo)): # pour toutes les arêtes à tester (on parcours un tableau de 3-uplets)
...
def bool_cycle_started(o,x,y,Chaine):
    """
    entrée : o,x,y 3 entiers correspondant aux coordonnées d'une arête, un plateau sous forme [H,V,C]
...
def best3_arete(arete1,arete2,arete3,chaine):
    """
    entrée : trois arêtes différentes sous forme [o,x,y], un plateau sous forme [H,V,C]
...
def trouve_meilleure_arête(chaine): 
    """
    entrée : un plateau sous forme [H,V,C]
    effet : dans le cas où aucune arête n'est sûre, trouve la meilleure arête ie celle qui permet d'ouvrir la chaîne la moins grande 
    sortie : un tableau [long,o,x,y] avec long la longueur de la chaine que ferme l'arête de coordonnées o,x,y
    """
    [a,b] = dim(chaine)
...
def trouves_les_meilleurs_arêtes(chaine): 
    """
    entrée : un plateau sous forme [H,V,C]
    effet : renvoie la liste des arêtes non prises des chaînes de longueur inférieure ou égale à 2
    sortie : une liste sous forme [long,o,x,y] avec long la longueur de la chaine que ferme l'arête de coordonnées (o,x,y)
    """
    [a,b] = dim(chaine)
    meilleure_arete = []
...
def aretes_gagnantes(plateau): 
    ...
    for i in range(dimension[0]-1):
        for j in range(dimension[1]-1):
            if(plateau[0][i][j] == 0 and plateau[0][i][j+1]==1 and plateau[1][i+1][j] == 1 and plateau[1][i][j] == 1 and plateau[2][i][j] == 0) :
                return (0,i,j)
 
    for i in range(dimension[0]-1):
        for j in range(dimension[1]-1):
            if(plateau[0][i][j] == 1 and plateau[0][i][j+1]==0 and plateau[1][i+1][j] == 1 and plateau[1][i][j] == 1 and plateau[2][i][j] == 0) :
                return (0,i,j+1)

    for i in range(dimension[0]-1):
        for j in range(dimension[1]-1):
            if(plateau[0][i][j] == 1 and plateau[0][i][j+1]==1 and plateau[1][i+1][j] == 1 and plateau[1][i][j] == 0 and plateau[2][i][j] == 0) :
                return (1,i,j)

    for i in range(dimension[0]-1):
        for j in range(dimension[1]-1):
            if(plateau[0][i][j] == 1 and plateau[0][i][j+1]==1 and plateau[1][i+1][j] == 0 and plateau[1][i][j] == 1 and plateau[2][i][j] == 0) :
                return (1,i+1,j)
...
def arete_sure(chaine):
    """
    entrée : un plateau sous forme [H,V,C]
    effet : renvoie une arête au pif qui ne donne pas de point à l'adversaire, et false sinon
    sortie : une arête sous forme (o,x,y) ou False 
    """
    [a,b] = dim(chaine)
    ...
    for _ in range (100):
        ...
        if o == 0 :
            x = randint(0,a-2)
            y = randint(0,b-1)
            if (plateau[0][x][y] == 0 and prochaine_arete(0,x,y,plateau) == False) :
                return (0,x,y)
        else :
            x = randint(0,a-1)
            y = randint(0,b-2)
            if (plateau[1][x][y] == 0 and prochaine_arete(1,x,y,plateau) == False) :
                return (1,x,y)

    # met une arête horizontale si aucun danger
    for i in range(a-1): 
        for j in range (b):
            if (plateau[0][i][j] == 0 and prochaine_arete(0,i,j,plateau) == False) :
                    return(0,i,j)

    #trace une arête verticale si aucun danger
    for i in range(a): 
        for j in range (b-1):
            if (plateau[1][i][j] == 0 and prochaine_arete(1,i,j,plateau) == False) :
                return(1,i,j)
    return False
                ...
                if(plateau[0][i][j] == 0 and plateau[0][i][j+1]==1 and plateau[1][i+1][j] == 1 and plateau[1][i][j] == 1 and plateau[2][i][j] == 0) :
                    if bool_cycle_started(0,i,j,deepcopy(plateau)) :
                        if longueur_chaine([(0,i,j)],deepcopy(plateau)) < 4:
                            ...
                            #print("dans le cycle de l'arête",(0,i,j), "la longueur vaut ",longueur_chaine([(0,i,j)],deepcopy(plateau)))
                            if time_activation :
                                sleep(delay)
            ...
            for j in range(dimension[1]-1):
                if(plateau[0][i][j] == 1 and plateau[0][i][j+1]==0 and plateau[1][i+1][j] == 1 and plateau[1][i][j] == 1 and plateau[2][i][j] == 0) :
                    if bool_cycle_started(0,i,j+1,deepcopy(plateau)) :
                        if longueur_chaine([(0,i,j+1)],deepcopy(plateau)) < 4:
                            #pour_y_voir_plus_clair(plateau)
                            #print("dans le cycle de l'arête",(0,i,j+1), "la longueur vaut ",longueur_chaine([(0,i,j+1)],deepcopy(plateau)))
                            if time_activation :
                                sleep(delay)
                ...
                if(plateau[0][i][j] == 1 and plateau[0][i][j+1]==1 and plateau[1][i+1][j] == 1 and plateau[1][i][j] == 0 and plateau[2][i][j] == 0) :
                    if bool_cycle_started(1,i,j,deepcopy(plateau)) :
                        if longueur_chaine([(1,i,j)],deepcopy(plateau))<4:
                            #pour_y_voir_plus_clair(plateau)
                            #print("dans le cycle de l'arête",(1,i,j), "la longueur vaut ",longueur_chaine([(1,i,j)],deepcopy(plateau)))
                            if time_activation :
                                sleep(delay)
            ...
            for j in range(dimension[1]-1):
                if(plateau[0][i][j] == 1 and plateau[0][i][j+1]==1 and plateau[1][i+1][j] == 0 and plateau[1][i][j] == 1 and plateau[2][i][j] == 0) :
                    if bool_cycle_started(1,i+1,j,deepcopy(plateau)) :
                        if longueur_chaine([(1,i+1,j)],deepcopy(plateau))<4:
                            #pour_y_voir_plus_clair(plateau)
                            #print("dans le cycle de l'arête",(1,i+1,j), "la longueur vaut ",longueur_chaine([(1,i+1,j)],deepcopy(plateau)))
                            if time_activation :
                                sleep(delay)
...
def que_next1_next2(plateau, couleur_bot):
    ...
    sortie : renvoie (False,[-1,-1,-1],[-1,-1,-1]) si il en a tracé au moins une, et (True,next_arete1,next_arete2) sinon \n
    /!\ l'algo peut très bien renvoyer (True,[-1,-1,-1],[-1,-1,-1]) ou (True,next_arete1,[-1,-1,-1]) si il y a moins de 2 arêtes gagnantes.
    """
    joue = 0
    dimension = dim(plateau)
    next_arete1 = [-1,-1,-1]
                    ...
                    if next_arete1 == [-1,-1,-1] :
                        ...
                        if next_arete1 != [0,i,j] :
                            if next_arete2 != [0,i,j] :
                                ...
                                next_arete3 = [0,i,j]
                                best_arete = best3_arete(next_arete1,next_arete2,next_arete3,plateau) 
                                if best_arete == 3 : 
                                    ...
                                    if time_activation :
                                        sleep(delay)
                                    draw_arete(clignote,plateau,next_arete4[0],next_arete4[1],next_arete4[2],couleur_bot)
                                    if next_arete4[0] == 0 :
                                        plateau[0][next_arete4[1]][next_arete4[2]] = 1
                                    else :
                                        plateau[1][next_arete4[1]][next_arete4[2]] = 1
                                    joue = joue + 1
                    ...
                    if next_arete1 == [-1,-1,-1] :
                        ...
                        if next_arete1 != [0,i,j+1] :
                            if next_arete2 != [0,i,j+1] :
                                ...
                                next_arete3 = [0,i,j+1]
                                best_arete = best3_arete(next_arete1,next_arete2,next_arete3,plateau) #disjonction de cas en cours d'écriture
                                if best_arete == 3 : 
                                    next_arete4 = next_arete3
                                elif best_arete == 2 :
                                    next_arete4 = next_arete2
...
def regle_double_case(plateau,couleur_bot):
    ...
    if peut_jouer : #règle de la double case
        if next_arete1 != [-1,-1,-1]: # si on a trouvé une arête qui nous fait gagner un point
            if next_arete2 == [-1,-1,-1] : # si on en a trouvé qu'une seule, on n'est pas dans un cycle.
                next_arete = next_arete1    
            else : # si on en a trouvé une 2e, on compare la longueur de leurs chaines respectives.
                long1 = longueur_chaine([(next_arete1[0],next_arete1[1],next_arete1[2])],deepcopy(plateau))
                long2 = longueur_chaine([(next_arete2[0],next_arete2[1],next_arete2[2])],deepcopy(plateau))
                
                if bool_cycle_started(next_arete1[0],next_arete1[1],next_arete1[2],plateau) and long1 == long2 :
                    next_arete = next_arete1
                    long = long1
                    while long > 4 : #on va se ramener à un cycle de longeur = 4
                        draw_arete(clignote,plateau,next_arete[0],next_arete[1],next_arete[2],couleur_bot)
                        plateau[next_arete[0]][next_arete[1]][next_arete[2]] = 1

                        if carre(plateau,couleur_bot) : #si l'arête que l'on vient de tracer nous fait bien gagner un point.
                            coordo = prochaine_arete(next_arete[0],next_arete[1],next_arete[2],plateau)
                            if coordo != False : 
                                (o,x,y) = coordo[0] # on extrait le 1er 3-uplet
                                while (o == 2 and prochaine_i < len(coordo)): #le 1er 3-uplet qui est bien un arête (o != 2)
                                    (o,x,y) = coordo[prochaine_i]
                                    prochaine_i += 1
                                next_arete = [o,x,y] #on modifie la valeur de next_arete
                                long = longueur_chaine([(next_arete[0],next_arete[1],next_arete[2])],deepcopy(plateau)) #et celle de long
                    
                    #on a maintenant un cycle de longueur 4
                    coordo = prochaine_arete(next_arete[0],next_arete[1],next_arete[2],deepcopy(plateau)) 
                    (o,x,y) = coordo[0] 
                    prochaine_i = 0
                    while (o == 2 and prochaine_i < len(coordo)):
                        (o,x,y) = coordo[prochaine_i] # on extrait le 1er 3-uplet tq o != 2
                        prochaine_i += 1

                    if plateau[o][x][y] == 0 :  # si l'arête n'est pas prise ou déjà testée on effectue la technique sur un cycle
            ...
            if peut_jouer : #il ne reste plus qu'une arête gagnante, on a réglé son compte à l'autre
                long = longueur_chaine([(next_arete[0],next_arete[1],next_arete[2])],deepcopy(plateau))
                ...
                if long != 2 :
                ...
                else :  # bon ici on place une arête aux coordonnées d'après, obtenues à l'aide le la fonction prochaine_arête.
                    coordo = prochaine_arete(next_arete[0],next_arete[1],next_arete[2],deepcopy(plateau)) 
                    (o,x,y) = coordo[0] 
                    prochaine_i = 0
                    while (o == 2 and prochaine_i < len(coordo)):
                    ...
                    if plateau[o][x][y] == 0 :  # si l'arête n'est pas prise ou déjà testée
                        if time_activation :
                            sleep(delay)
                        draw_arete(clignote,plateau,o,x,y,couleur_bot)
                        plateau[o][x][y] = 1
...
def coupe_chaine_2(plateau):
    ...
    entrée : un plateau sous forme [H,V,C]
    effet : 
        Si parmis les arêtes disponibles, une ferme une chaine de longueur exactement 2 en la coupant en 2, on renvoie celle-ci \n
    ...
    for tab in liste_arete_en_jeu :
        if tab[0] == 2 :
            if (tab[1] == 0 and tab[3] != dimension[1]-1 and tab[3] != 0 and (tab[2]==dimension[0]-1 or tab[2]==0)) :
                res = (tab[1],tab[2],tab[3])
            else :
                if (tab[1] == 1 and tab[2] !=dimension[0]-1 and tab[2] !=0 and (tab[3]==dimension[1]-1 or tab[3]==0)) :
                    res = (tab[1],tab[2],tab[3])
    if res == (-1,0,0) :
...
def complète_chaine(coordo,Chaine):
    ...
    effet : rempli la chaine fermée par l'arête passée en entrée, ie modifie le plateau en conséquence mais ne modifie pas l'écran
    sortie : ()
    """
    for i in range(len(coordo)): # pour toutes les arêtes à tester (on parcours un tableau de 3-uplets)
        (o,x,y) = coordo[i] # on extrait le ie 3-uplet
...
def arete_sure_existence(plateau):
    """
    entrée : un plateau sous forme [H,V,C]
    effet : renvoie True ssi il existe une arête sûre (une arête qui ne donne pas de points à l'adversaire)
    sortie : un booléen
    """
    [a,b] = dim(plateau)
    chaine = deepcopy(plateau)
...
def test_grundy(plateau):
    """
    entrée : un plateau sous forme [H,V,C]
    effet : determine si on peut appliquer le théorème de sprague-grundy sur le plateau donné
    sortie : un booléen
    """
    chaine = deepcopy(plateau)
...
def nombre_grundy(plateau):
    ...
    explication calcul du nombre de grundy :
        ...
        calcul en pratique :
             pour chaque arête interessante (d'une chaine de longueur < 3), complète la chaine et ajoute 1
    """
    liste_arete_en_jeu = trouves_les_meilleurs_arêtes(plateau)
    chaine = deepcopy(plateau)
    nb_grundy = 0
    arete_gagnante = aretes_gagnantes(chaine)
    while(arete_gagnante != False):
...
def sprague_grundy(clignote,plateau,c):
            ...
            on calcule le nombre de grundy sur ce plateau :
                s'il vaut 1 : le joueur qui doit jouer gagne (ici nous) :
                    combien la longueur de la chaine minimale vaut-elle ?
                        si elle vaut 1 : on prend l'arête correspondante
                        si elle vaut 2 : on prend une arête qui coupe en 2 une chaine de 2 
                s'il vaut 0 : le joueur qui doit jouer perd (ici nous) :
                    on prend une arête d'une chaine de longueur 1, en espérant que le joueur adverse ouvre une chaine de 2 sans la couper en 2
         si on peut prendre un point :
             si la chaine ouverte est de longueur 1, on a pas de choix:
                on prend ce point et on relance l'algo sur le plateau modifié en conséquence
             sinon la chaine ouverte est de longueur 2, on a le choix de prendre un point où de le laisser à l'adversaire:
    ...
    if arete_gagnante == False : #NON on ne peut pas prendre de point 
