...
from affichage import dim, draw_arete, draw_carre
                    ...
                    if plateau[1][i + 1][j] == 1 and plateau[1][i][j] == 1:
                        if plateau[2][i][j] == 0:
                            if bool_cycle_started(0, i, j + 1, deepcopy(plateau)):
                                if (
                                    longueur_chaine([(0, i, j + 1)], deepcopy(plateau))
                                    < 4
                                ):
                                    # pour_y_voir_plus_clair(plateau)
                                    if time_activation:
                                        sleep(delay)
                                    draw_arete(
                                        clignote, plateau, 0, i, j + 1, couleur_bot
                                    )
                                    joue = joue + 1
                                    plateau[0][i][j + 1] = 1
                    ...
                    if plateau[1][i + 1][j] == 0 and plateau[1][i][j] == 1:
                        if plateau[2][i][j] == 0:
                            if bool_cycle_started(1, i + 1, j, deepcopy(plateau)):
                                if (
                                    longueur_chaine([(1, i + 1, j)], deepcopy(plateau))
                                    < 4
                                ):
                                    # pour_y_voir_plus_clair(plateau)
                                    if time_activation:
                                        sleep(delay)
                                    draw_arete(
                                        clignote, plateau, 1, i + 1, j, couleur_bot
                                    )
                                    joue = joue + 1
                                    plateau[1][i + 1][j] = 1
    if joue == 0:
...
def regle_double_case(clignote, plateau, couleur_bot):
    ...
    # on trace celles-ci
    (peut_jouer, next_arete1, next_arete2) = que_next1_next2(
        clignote, plateau, couleur_bot
    )
    ...
    if peut_jouer:  # règle de la double case
        if next_arete1 != [-1, -1, -1]:
            ...
            if next_arete2 == [-1, -1, -1]:
                ...
                long1 = longueur_chaine(
                    [(next_arete1[0], next_arete1[1], next_arete1[2])],
                    deepcopy(plateau),
                )
                long2 = longueur_chaine(
                    [(next_arete2[0], next_arete2[1], next_arete2[2])],
                    deepcopy(plateau),
                )

                if (
                    bool_cycle_started(
                        next_arete1[0], next_arete1[1], next_arete1[2], plateau
                    )
                    and long1 == long2
                ):
                    next_arete = next_arete1
                    ...
                    while long > 4:  # on va se ramener à un cycle de longeur = 4
                        ...
                        if carre(plateau, couleur_bot):
                            ...
                            coordo = prochaine_arete(
                                next_arete[0], next_arete[1], next_arete[2], plateau
                            )
                            prochaine_i = 0
                            if coordo:
                                ...
                                long = longueur_chaine(
                                    [(next_arete[0], next_arete[1], next_arete[2])],
                                    deepcopy(plateau),
                                )

                    # on a maintenant un cycle de longueur 4
                    coordo = prochaine_arete(
                        next_arete[0], next_arete[1], next_arete[2], deepcopy(plateau)
                    )
                    (o, x, y) = coordo[0]
                    prochaine_i = 0
                    while o == 2 and prochaine_i < len(coordo):
